...
<script lang="ts">
	...
	import type { CssClasses } from './types.d.ts';
	...
	const cThumb = 'w-1/2 h-full scale-[0.8] transition-all duration-[200ms] shadow';
	...
	let classesBase = $derived(`${cBase} ${rounded} ${classesDisabled} ${classNames ?? ''}`);
	let classesLabel = $derived(`${cLabel}`);
	let classesTrack = $derived(`${cTrack} ${border} ${rounded} ${trackSize} ${cTrackActive}`);
	let classesThumb = $derived(`${cThumb} ${rounded} ${cThumbBackground} ${cThumbPos}`);
